...
try:
    with open(os.path.join(os.path.dirname(__file__), 'data', 'ver')) as f:
...
def main(arguments):
    ...
    parser_outfiles.add_argument(
        ...
        help="file containing sequences dropped due to ambiguities or cluster size")
    parser_outfiles.add_argument(
    ...
    parser.add_argument('-k', '--keep-abundances', action='store_true',
                        default=False,
                        help='retain abundance annotation in sequence names (-w/--seeds only)')
    parser.add_argument('--min-mass', type=int, default=None, metavar='N',
    ...
    if args.specimen_map:
        ...
        tmpfile = ntf(
            'w', prefix='single-specimen', suffix='.fasta', dir=args.tmpdir, delete=False)
        specimen_map = defaultdict(lambda: 'single-specimen')
        raw_reads = defaultdict(lambda: tmpfile)
    ...
    with ntf('w', prefix='d0all-', suffix='.fasta', dir=args.tmpdir, delete=delete) as d0all, \
        ...
        # calculate abundances for each specimen and generate output
        ...
        for seq, line in zip(fastalite(swarm_out), clusters):
            ...
            otu_rep_mass = sum(counts)
            if args.abundances:
                writer.writerow([otu_rep, otu_rep, otu_rep_mass])
            ...
            # finally, write the OTU seed
            if args.keep_abundances:
                args.seeds.write('>{}_{}\n{}\n'.format(otu_rep, otu_rep_mass, seq.seq))
            else:
